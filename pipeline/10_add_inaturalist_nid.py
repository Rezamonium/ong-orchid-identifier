"""
Add iNaturalist Needs-ID Photos — ONG Orchid Identifier v2

Source: E:/Claude Code/iNaturalist scrapping/inaturalist_orchids_needs_id/
Structure: {Genus}_{epithet}/ -> photos already named:
  {Genus}_{Genus}_{epithet}_{obs_id}_{photo_idx}_{username}_inaturalist.jpg

Output name (add _NID suffix to mark as unconfirmed):
  {Genus}_{Genus}_{epithet}_{obs_id}_{photo_idx}_{username}_inaturalist_NID.jpg

Notes:
  - NID photos only used for genus classifier training (Stage 1)
  - Included in train/val/test splits for genus-level training
  - Marked separately so they can be excluded from FAISS index if needed
  - Species label taken from folder name (genus-level confidence only)
"""
...
# ── Paths ─────────────────────────────────────────────────────────────────────
INAT_NID   = Path("E:/Claude Code/iNaturalist scrapping/inaturalist_orchids_needs_id")
...
# ════════════════════════════════════════════════════════════════════
# STEP 1: Copy photos with _NID suffix
# ════════════════════════════════════════════════════════════════════
print("Step 1: Copying iNaturalist Needs-ID photos (adding _NID suffix)...")
...
for sp_dir in sorted(INAT_NID.iterdir()):
    ...
    for img in imgs:
        new_name = img.stem + "_NID" + img.suffix
    ...
    log_rows.append({"genus": genus, "species": species_str, "photos": len(imgs)})

print(f"  Copied  : {nid_total:,} photos")
...
# ════════════════════════════════════════════════════════════════════
# STEP 3: Append to image_type_labels.csv as 'live'
# ════════════════════════════════════════════════════════════════════
...
print(f"  Added {len(new_entries):,} NID entries as 'live'")
...
# ── Write report ──────────────────────────────────────────────────────────────
report_path = DATA_DIR / "inaturalist_nid_report.txt"
with open(report_path, "w", encoding="utf-8") as f:
    f.write("iNaturalist Needs-ID Photos — Integration Report\n")
    f.write("=" * 60 + "\n\n")
    f.write(f"Source  : {INAT_NID}\n")
    f.write(f"Folders : {len(log_rows)}\n")
    f.write(f"Photos  : {nid_total:,}\n\n")
